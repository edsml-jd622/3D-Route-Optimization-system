...
class Encoder(nn.Module):
    ...
    def forward(self, embedded_inputs,
        ...
        #In lstm layer, the first dimension should be LEN_SEQ, so exchange the position between batch_size and len_seq.
        embedded_inputs = embedded_inputs.permute(1, 0, 2)

        outputs, hidden = self.lstm(embedded_inputs, hidden)

        #return should change the dimension back.
...
class PointerNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.embedding_dim = embedding_dim
        self.bidir = bidir
        self.embedding = nn.Linear(cities_num, embedding_dim)
        ...
        #Create a random parameters for the initial input of decoder, the size of it is embedding_dim
    ...
    def forward(self, inputs):
        ...
        #Make decoder_input0's first dimension as batch_size (just copy the initial decoder_input0 BATCH_SIZE times)
        ...
        if self.bidir:
            decoder_hidden0 = (torch.cat((encoder_hidden[0][-2], encoder_hidden[0][-1]), dim=-1),
                               torch.cat((encoder_hidden[1][-2], encoder_hidden[0][-1]), dim=-1))
        else:
